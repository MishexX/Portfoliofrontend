...
export default function ExperienceEducation() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(async ([expRes, eduRes]) => {
        try {
          const exp = await expRes.json();
          const edu = await eduRes.json();
          
          // Ensure we have arrays, even if API returns error objects
          setExperienceItems(Array.isArray(exp) ? exp : []);
          setEducationItems(Array.isArray(edu) ? edu : []);
          setLoading(false);
        } catch (error) {
      ...
      <div className="container mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            {loading ? <div className="text-purple-400 text-center">Lade...</div> : error ? <div className="text-red-400 text-center">{error}</div> : (
              <div className="space-y-4">
                {!Array.isArray(experienceItems) || experienceItems.length === 0 ? <div className="text-gray-400 text-center">Keine Einträge vorhanden.</div> : experienceItems.map((item, index) => (
                  <div
                    key={item._id || index}
          ...
          <div>
            ...
            {loading ? <div className="text-purple-400 text-center">Lade...</div> : error ? <div className="text-red-400 text-center">{error}</div> : (
              <div className="space-y-4">
                {!Array.isArray(educationItems) || educationItems.length === 0 ? <div className="text-gray-400 text-center">Keine Einträge vorhanden.</div> : educationItems.map((item, index) => (
                  <div
                    key={item._id || index}
                    className="p-4 md:p-6 bg-[#140C1C] rounded-lg hover:border-l-4 hover:border-purple-500 transition-all"
                  >
