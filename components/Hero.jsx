...
export default function Hero({ showStats = false }) {
  ...
  const handleRequestDownload = async (e) => {
    ...
    if (!email.trim()) {
      setFormStatus("error");
      setFormMsg("Bitte gib eine E-Mail-Adresse ein.");
      return;
    }
    ...
    if (!emailRegex.test(email.trim())) {
      setFormStatus("error");
      setFormMsg("Bitte gib eine gültige E-Mail-Adresse ein.");
      return;
    }
    
    if (!privacyAccepted) {
      setFormStatus("error");
      setFormMsg("Bitte akzeptiere die Datenschutzerklärung.");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormMsg("Download-Link wurde per E-Mail versendet. Ein neuer Link wird in 10 Minuten automatisch gesendet!");
        setEmail("");
      } else {
        setFormStatus("error");
