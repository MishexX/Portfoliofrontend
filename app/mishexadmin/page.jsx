...
export default function AdminLogin() {
  ...
  async function handleProfileUpload(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProfileError(errorData.error || "Fehler beim Hochladen.");
      }
    } catch (error) {
      console.error('Upload exception:', error);
      setProfileError("Fehler beim Hochladen: " + error.message);
    }
    setProfileLoading(false);
  }
  ...
  async function handleSkillSubmit(e) {
    ...
    try {
      console.log('Submitting skill form:', skillForm);
      ...
      if (skillForm.icon instanceof File) {
        ...
        console.log('Icon file added:', skillForm.icon.name);
      }
      
      const method = skillEditId ? "PUT" : "POST";
      const url = skillEditId
        ? `/api/skills/${skillEditId}`
        : "/api/skills";
      ...
      if (res.ok) {
        ...
        setSkillEditId(null);
        fetchSkills();
        setSkillError(""); // Clear any previous errors
      } else {
        let errorData;
        try {
          const responseText = await res.text();
        ...
        setSkillError(errorData.message || "Fehler beim Speichern");
      }
    } catch (error) {
      console.error('Skill submit exception:', error);
      setSkillError("Fehler beim Speichern: " + error.message);
    }
  }
  ...
  async function fetchCvStats() {
    ...
    const res = await fetch("/api/cv-download-stats");
